package com.swf.monitor.multi.datasource;
...
/**多数据源注册文件   这个注入是最核心的部分
 * 动态创建多数据源注册到Spring中
 *接口：BeanDefinitionRegistryPostProcessor只要是注入bean,在上一节介绍过使用方式；
 	接口：接口 EnvironmentAware 重写方法 setEnvironment可以在工程启动时，获取到系统环境变量和application配置文件中的变量。 这个第24节介绍过.
		...
		System.out.println("MultipleDataSourceBeanDefinitionRegistryPostProcessor.postProcessBeanFactory()");
	       ...
	       if(!dataSourceMap.isEmpty()){
	           ...
	           BeanDefinition bd = null;
	            ...
	            for (Entry<String, Map<String, Object>> entry : dataSourceMap.entrySet()) {//给数据源设定信息
	       ...
	       try {
	           if(!dataSourceMap.isEmpty()){
	              ...
	              for(Entry<String,Map<String,Object>> entry:dataSourceMap.entrySet()){
	                  ...
	                  registerBean(registry, entry.getKey(),Class.forName(type.toString()));//(Class<? extends DataSource>)Class.forName(type.toString()) 当前此处修改
	              }
	           }
	...
	public void setEnvironment(Environment environment) {
		...
		System.out.println("MultipleDataSourceBeanDefinitionRegistryPostProcessor.setEnvironment()");
	       ...
	       for(String dsPrefix:dsPrefixsArr){
	           ...
	           if(!MapUtils.isEmpty(dsMap)) {//只有获取到的map存在值才加入到数据源map中
